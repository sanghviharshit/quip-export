...
class QuipProcessor {
            ...
            if(message.files) {
                for(const file of message.files) {
                    ...
                    if(mimeType && mimeType.startsWith('image/')) {
                        ...
                        text += await this._processFile(imageHtml, fileMatch, path, this.options.embeddedImages, quipThread.thread.created_usec, quipThread.thread.updated_usec);
                    } else {
                        //file
                        let fileName = file.name? file.name : file.hash;
                        const fileHtml = `<br/><a href="RELATIVE_PATH">${fileName}</a><br/>`;
                        text += await this._processFile(fileHtml, fileMatch, path, false, quipThread.thread.created_usec, quipThread.thread.updated_usec);
                    }
    ...
    async _processDocumentThreadDocx(quipThread, path) {
        ...
        const fullPath = pathModule.join(this.destinationFolder, 'quip-export', path, fileName);
        if (fs.existsSync(fullPath)) {
            this.logger.debug(`File already exists, skipping download: ${fullPath}`);
            return;
        }
        const docx = await this.quipService.getDocx(quipThread.thread.id);
        if(docx) {
            this.saveCallback(docx, fileName, 'BLOB', path, quipThread.thread.created_usec, quipThread.thread.updated_usec);
        }
    }
    ...
    async _processDocumentThreadXlsx(quipThread, path) {
        ...
        if(xlsx) {
            this.saveCallback(xlsx, fileName, 'BLOB', path, quipThread.thread.created_usec, quipThread.thread.updated_usec);
        }
    }
    ...
    async _processDocumentThread(quipThread, path) {
        ...
        for(const image of matches) {
            threadHtml = await this._processFile(threadHtml, image, path, this.options.embeddedImages, quipThread.thread.created_usec, quipThread.thread.updated_usec);
        }
        ...
        for(const link of matches) {
            threadHtml = await this._processFile(threadHtml, link, path, false, quipThread.thread.created_usec, quipThread.thread.updated_usec);
        }
        ...
        this.saveCallback(
            wrappedHtml,
            sanitizeFilename(`${quipThread.thread.title.trim()}.html`),
            'THREAD',
            path,
            quipThread.thread.created_usec,
            quipThread.thread.updated_usec
        );
    }
    ...
    async _processFile(html, file, path, asImage = false, created_usec, updated_usec) {
        let fileName;
        if(file.fileName) {
            fileName = file.fileName.trim();
        } else {
            // We'll only know the extension after download, but for most cases, use blobId
        ...
        if(blob) {
            if(asImage) {
                ...
                // Try to get extension from blob type if not already set
                if(!file.fileName) {
